...
function appml(appmlname) {
  ...
  for (i = 0; i < appmlObjects.length; i += 1) {
    if (appmlname == appmlObjects[i].appName) {
      return appmlObjects[i];
    }
  }
}
(function () {
...
function AppML(container, appsrc, appmlname) {
  ...
  this.invokeAppML = function () {
    ...
    for (i = 0; i < z.length; i += 1) {
      ...
      if (z[i].getAttribute("appml-include-html")) {
        ...
        xmlhttp = this.xmlHttp(
          z[i].getAttribute("appml-include-html"),
          "",
          "GET",
          false
        );
    ...
    for (i = 0; i < z.length; i += 1) {
      //Manage Appml Applications:
      if (
        z[i].hasAttributes &&
        ((att = z[i].getAttribute("appml-controller")) !== null ||
          (att = z[i].getAttribute("appml-data")) !== null)
      ) {
  ...
  this.getData = function (nav, id) {
    ...
    if (navigate === "same") {
      fromrec = this.data.fromRec;
    }
    if (this.dataSource) {
      if (
        this.dataSource.indexOf(".") == -1 &&
        this.dataSource.indexOf("local?") == -1
      ) {
        errmsg = "The datasource " + this.dataSource + " does not exist";
      }
      if (errmsg == null && this.dataSource.indexOf("?model=") > -1) {
        this.data.records = [];
        if (this.appmlID === "NULL") {
          fromrec = 0;
        }
        this.app = {};
        ...
        if (this.dataSource.substr(0, 12) === "local?model=") {
          ext = ".js";
          if (this.dataSource.substr(12).indexOf(".") > -1) {
            ext = "";
          }
      ...
      if (errmsg == null) {
        ...
        if (xmlhttp.status == 404) {
          errmsg = "The page cannot be found: " + this.dataSource;
        }
        if (this.data.error) {
  ...
  this.displayHTML = function () {
    ...
    for (j = 0; j < l; j += 1) {
      ...
      if (
        ...
        for (x in repeatObj) {
          ...
          rowClone = this.needleInHaystack(
            rowClone,
            "element",
            repeatX,
            repeatObj[x]
          ...
          for (ii = 0; ii < a.length; ii += 1) {
            a[ii].value = this.needleInHaystack(
              a[ii],
              "attribute",
              repeatX,
              repeatObj[x]
    ...
    if (this.data.records) {
      for (i = 1; i <= this.data.records.length; i += 1) {
        for (j = 0; j < 3; j += 1) {
          if (j === 0) {
            a = this.html.getElementsByTagName("select");
          }
          if (j === 1) {
            a = this.html.getElementsByTagName("input");
          }
          if (j === 2) {
            a = this.html.getElementsByTagName("textarea");
          }
          l = a.length;
          for (ii = 0; ii < l; ii += 1) {
            ...
            for (iii = 0; iii < ll; iii += 1) {
              if (
                a[ii].id.toLowerCase() ===
                this.data.itemNames[iii].toLowerCase()
              ) {
                a[ii].value =
                  this.data.records[i - 1][this.data.itemNames[iii]];
              }
            }
          }
        }
    ...
    while (pos1 > -1) {
      ...
      for (i = 0; i < needle.length; i += 1) {
        ...
        if (this.display.value == undefined) {
          try {
            cc = needle[i].split(".");
            if (cc[0] == repeatX) {
              this.display.value = x[cc[1]];
              this.display.name = cc[1];
            }
          } catch (er) {
            this.display.value = undefined;
            this.display.name = needle[i];
      ...
      if (this.display.value != undefined) {
        ...
        if (typ == "attribute") {
          ...
          if (replaceFast === -1) {
            replaceFast = checkReplaceInnerHTML(rowClone);
          }
      ...
      for (iii = 0; iii < l; iii += 1) {
        if (b[iii].outerHTML.indexOf(r) > -1) {
          if (b[iii].tagName !== "TBODY" && b[iii].tagName !== "TR") {
            if (b[iii].innerHTML.indexOf(r) > -1) {
              b[iii].innerHTML = b[iii].innerHTML.replace(r, result);
            }
          } else {
            if (b[iii].hasAttributes()) {
              ...
              for (iiii = 0; iiii < lll; iiii += 1) {
                if (bb[iiii].value.indexOf(r) > -1) {
                  bb[iiii].value = bb[iiii].value.replace(r, result);
                }
              }
            }
          }
  ...
  this.getElement = function (id, parent) {
    //Search within a specified element, or the appml container if no parent is defined. Return the element with the specified id:
    var parentObj = parent || this.container;
    if (!parentObj) {
      return false;
    }
    var y = parentObj.getElementsByTagName("*"),
      l = y.length,
      i,
      z = id.toUpperCase();
    for (i = 0; i < l; i += 1) {
      if (y[i].id.toUpperCase() === z) {
        return y[i];
      }
    }
  };
  this.getElementsByAttribute = function (x, att) {
    //Search within a specified element, and return all elements with the specified attribute
    var arr = [],
      arrCount = -1,
      i,
      l,
  ...
  this.setCommandsClick = function () {
    ...
    if (cc) {
      cc.onclick = function () {
        obj.getElement("appml_filtercontainer").style.display = "none";
      };
    }
    ...
    from = this.data.fromRec;
    to = this.data.toRec;
    count = this.data.totalRecCounter;
    if (
      this.displayType.toUpperCase() != "LIST" &&
      this.displayType.toUpperCase() != "FORM"
    ) {
      return false;
    }
    for (i = 1; i <= 4; i += 1) {
      ...
      if (i === 4) {
        x = "last";
        if (
          (to === count && Number(from) - 1 === Number(to) - Number(fc)) ||
          Number(count) < Number(fc)
        ) {
          typ = "disable";
        }
      }
    ...
    recout =
      ...
      to +
      " " +
      this.translate("APPML_MESSAGE_OF") +
      " " +
      count;
    if (this.maxRecords !== 0) {
      if (Number(count) >= Number(this.maxRecords)) {
        recout = recout + "+";
      }
    }
  ...
  this.disableFNPL = function (nav) {
    if (this.getElement("appmlbtn_" + nav)) {
      ...
      this.getElement("appmlbtn_" + nav).onclick = function () {
        return false;
      };
    }
  };
  ...
  this.translate = function (txt) {
    var patt;
    patt = /APPML_ERR_USN_OR_PWD_REQ/g;
    txt = txt.replace(patt, "Username or password required");
    patt = /APPML_ERR_ACTION_REQ/g;
    txt = txt.replace(patt, "Action required");
    patt = /APPML_ERR_MODEL_REQ/g;
    txt = txt.replace(patt, "Model required");
    patt = /APPML_ERR_MODEL_ERR/g;
    txt = txt.replace(patt, "Error in Model");
    patt = /APPML_ERR_NO_LOGIN/g;
    txt = txt.replace(patt, "You are not logged in");
    patt = /APPML_ERR_NOT_AUTHORIZED/g;
    txt = txt.replace(patt, "You are not authorized to perform this action");
    patt = /APPML_ERR_DATASOURCE_REQ/g;
    txt = txt.replace(patt, "Datasource required");
    patt = /APPML_ERR_KEYFIELD_REQ/g;
    txt = txt.replace(patt, "Keyfield required");
    patt = /APPML_ERR_MAINTABLE_REQ/g;
    txt = txt.replace(patt, "Maintable required");
    patt = /APPML_ERR_ILLEGAL_ACTION/g;
    txt = txt.replace(patt, "Illegal action");
    patt = /APPML_ERR_DATAMODEL/g;
    txt = txt.replace(patt, "Error in datamodel");
    patt = /APPML_ERR_INPUT_MIN/g;
    txt = txt.replace(patt, "Minimum value error");
    patt = /APPML_ERR_INPUT_MAX/g;
    txt = txt.replace(patt, "Maximum value error");
    patt = /APPML_ERR_INPUT_REQ/g;
    txt = txt.replace(patt, "Required value error");
    patt = /APPML_ERR_ILLEGAL_QUERY/g;
    txt = txt.replace(patt, "Illegal query error");
    patt = /APPML_ERR_UKNOWN_DB/g;
    txt = txt.replace(patt, "Unknown database");
    patt = /APPML_ERR_UKNOWN_DB_FIELD/g;
    txt = txt.replace(patt, "Unknown database field");
    patt = /APPML_ERR_INVALID_KEY/g;
    txt = txt.replace(patt, "Invalid key");
    patt = /APPML_ERR_ERROR/g;
    txt = txt.replace(patt, "Error");
    patt = /APPML_MESSAGE_RECORD_UPDATED/g;
    txt = txt.replace(patt, "Record updated");
    patt = /APPML_MESSAGE_RECORD_ADDED/g;
    txt = txt.replace(patt, "Record added");
    patt = /APPML_MESSAGE_RECORD_DELETED/g;
    txt = txt.replace(patt, "Record deleted");
    patt = /APPML_MESSAGE_OF/g;
    txt = txt.replace(patt, "of");
    return txt;
  };
  ...
  this.setError = function (n, desc) {
    ...
    if (this.controller) {
      this.message = "error";
      if (window[this.controller](this) == -1) {
        return -1;
      }
    }
  ...
  this.putRecord = function (param) {
    ...
    if (action === "UPDATE") {
      if (this.data.records[0][this.data.keyField] === "") {
        action = "ADD";
      }
    }
    this.app = {};
    ...
    if (this.upd && this.upd.error) {
      errmsg = this.upd.error;
    }
    if (errmsg != "") {
      this.displayMessage(errmsg);
      return -1;
    ...
    if (this.list) {
      this.list.data.totalRecCounter = 0;
      this.list.fnpl("same");
    }
  };
  ...
  function manageTemplate(obj) {
    if (obj.container) {
      obj.template = obj.container.cloneNode(true);
    }
  }
  ...
  function addClass(x, addclass) {
    ...
    if (clsnm.indexOf(addclass + " ") === 0) {
      return false;
    }
    if (clsnm.indexOf(" " + addclass) > -1) {
      if (clsnm.indexOf(addclass) + addclass.length === clsnm.length) {
        return false;
      }
      if (clsnm.indexOf(" " + addclass + " ") > -1) {
        return false;
  ...
  function checkReplaceInnerHTML(elmnt) {
    // checking how the browser replaces the innerHTML, IE has problems replacing the innerHTML of some elements, like TR
    var cc,
      cc1,
      cc2,
      cc3,
  ...
  this.appmlQuery = function () {
    ...
    for (i = 0; i < l; i += 1) {
      ...
      if (this.getElement("appml_query_" + qfname, queryElmnt)) {
        this.getElement("appml_query_" + qfname, queryElmnt).value = qvalue;
      }
      if (qoper !== "") {
        if (this.getElement("appml_operator_" + qfname, queryElmnt)) {
          this.getElement("appml_operator_" + qfname, queryElmnt).value = qoper;
        }
      }
    }
    if (!this.data.filteritems || this.data.filteritems.length === 0) {
      this.getElement("appml_filter", queryElmnt).style.display = "none";
    }
    ...
    for (i = 0; i < l; i += 1) {
      ...
      this.getElement(
        "appml_orderdirection_" + odir,
        queryElmnt
      ).checked = true;
    }
    if (
      this.getElement("appml_orderby", queryElmnt) &&
      (!this.data.sortitems || this.data.sortitems.length === 0)
    ) {
      this.getElement("appml_orderby", queryElmnt).style.display = "none";
    }
  ...
  this.makeQuery = function () {
    ...
    for (i = 0; i < l; i += 1) {
      if (x[i].id) {
        if (x[i].id.substr(0, 12) === "appml_query_") {
          ...
          ftype = this.getElement(
            "appml_datatype_" + fname
          ).value.toLowerCase();
          if (fvalue.length > 0) {
            qcount += 1;
            ...
            if (ftype === "date") {
              ...
              try {
                if (
                  ...
                  fromDate = new Date(
                    reformatDate(fvalue, this.data.dateFormat)
                  );
                  dateOK = 1;
                }
                if (fvalue.length === 7 || fvalue.length === 6) {
                  fromDate = new Date(
                    reformatDate("01/" + fvalue, this.data.dateFormat)
                  );
    ...
    for (i = 0; i < l; i += 1) {
      if (x[i].id && x[i].id === "appml_orderselect") {
        ...
        xx = this.getElement("appml_filtercontainer").getElementsByTagName(
          "input"
        );
        ll = xx.length;
        for (ii = 0; ii < ll; ii += 1) {
          if (xx[ii].name === "appml_orderdirection") {
            odirvalue = "asc";
            if (xx[ii].checked === true) {
              odirvalue = xx[ii].value;
            }
  ...
  this.resetQuery = function () {
    var i,
      l,
      x = this.getElement("appmlquery").getElementsByTagName("INPUT");
    l = x.length;
    for (i = 0; i < l; i += 1) {
      if (x[i].id.substr(0, 12) === "appml_query_") {
  ...
  this.setOrder = function (field, direction) {
    ...
    if (!direction || direction === "") {
      if (this.orderBys[0] === field) {
        if (this.orderByDirections[0] === "asc") {
          dir = "desc";
        }
      }
    }
  ...
  function localStorage(obj, cb) {
    ...
    if (!w3schoolsWebSQLOK) {
      obj.displayMessage(
        "Your browser does not support Local Storage Database (WebSQL)."
      );
      return;
    }
    if (obj.app.action === "GET") {
      xmlhttp = obj.xmlHttp(
        obj.appFile + "?r=" + Math.random(),
        "",
        "GET",
        false
      );
